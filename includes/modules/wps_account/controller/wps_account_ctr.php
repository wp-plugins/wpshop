...
class wps_account_ctr {
	...
	*/
	...
	function __construct() {
		...
		wp_enqueue_script( 'wps_forgot_password_js', WPS_ACCOUNT_URL.'wps_account/templates/frontend/js/wps_forgot_password.js' );
		wp_enqueue_script( 'wps_login_js', WPS_ACCOUNT_URL.'wps_account/templates/frontend/js/wps_login.js' );
		wp_enqueue_script( 'wps_signup_js', WPS_ACCOUNT_URL.'wps_account/templates/frontend/js/wps_signup.js' );
		wp_enqueue_script( 'wps_account_js', WPS_ACCOUNT_URL.'wps_account/templates/frontend/js/wps_account.js' );
	...
	function display_signup() {
		...
		if ( get_current_user_id() == 0 ) {
			...
			ob_start();
			require( $this->get_template_part( "frontend", "signup/signup") );
			$output = ob_get_contents();
			ob_end_clean();
		}
		return $output;
	...
	function display_account_informations() {
		...
		if( get_current_user_id() != 0 ) {
						...
						if( !empty($attributes_sections) ) {
							foreach( $attributes_sections as $attributes_section ) {
								...
								foreach( $attributes_details as $attributes_detail ) {
									...
									ob_start();
									require( $this->get_template_part( "frontend", "account/account_informations_element") );
								...
								ob_start();
								require( $this->get_template_part( "frontend", "account/account_informations_group_element") );
			...
			ob_start();
			require_once( $this->get_template_part( "frontend", "account/account_informations") );
			$output = ob_get_contents();
			ob_end_clean();
		}
		return $output;
	...
	function account_informations_form() {
		...
		if ( get_current_user_id() != 0 ) {
			...
			ob_start();
			require( $this->get_template_part( "frontend", "account/account_form") );
			$output = ob_get_contents();
			ob_end_clean();
		}
		return $output;
