	...
	<div>
		...
		<div class="wps-boxed" style="min-height : 127px">
			...
			<div class="wps-cart-resume-alignRight">
				...
				<p><?php _e( 'Shipping cost', 'wpshop'); ?>
				<?php echo ( ( !empty($shipping_price_from) && empty( $_SESSION['shipping_address'] ) ) ? '<br/><i>('.__( 'From', 'wpshop').')</i>' : '' ); ?> 
				<span class="wps-alignRight">
				<?php if( $cart_type != 'admin-panel' ) : ?>
				<strong><?php echo wpshop_tools::formate_number( $shipping_cost_ati ); ?></strong> <?php echo $currency; ?>
				<?php else : ?>
					<?php if( $cart_content['order_status'] == 'awaiting_payment' ) : ?>
						<span class="wps-form-group"><span class="wps-form"><input type="text" size="5" value="<?php echo wpshop_tools::formate_number( $shipping_cost_ati ); ?>" id="wps-orders-shipping-cost" class="wps-error" style="text-align : right" /></span></span>
					<?php else : ?>
				...
				<?php if( $cart_option == 'full_cart' || $cart_option == 'simplified_et' ) : ?>
				<p><?php _e( 'Total ET', 'wpshop'); ?>
				<?php if( $cart_option == 'simplified_et' && $cart_type == 'admin-panel' ) : ?>
					<span class="wps-form-group"><span class="wps-form"><input type="text" size="5" value="<?php echo wpshop_tools::formate_number( $shipping_cost_ati ); ?>" id="wps-orders-shipping-cost" class="wps-error" style="text-align : right" /></span></span>
				<?php else : ?>
					<span class="wps-alignRight"><strong><?php echo wpshop_tools::formate_number( $total_et ); ?></strong> <?php echo $currency; ?></span>
				<?php endif; ?>
				</p>
				<?php endif; ?>
				...
				<?php if( $cart_type == 'admin-panel' ) : ?>
					<p>
						...
						<span class="wps-alignRight">
							<?php if( $cart_content['order_status'] == 'awaiting_payment' || empty($cart_content['order_status']) ) : ?>
...
<?php if( !empty($cart_type) && $cart_type == 'admin-panel' && ( $cart_content['order_status'] == 'awaiting_payment' || empty($cart_content['order_status']) ) ) : ?>
