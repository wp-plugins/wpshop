...
if ( !class_exists("wpshop_account") ) {
		class wpshop_account {
			...
			function __construct() {
				add_shortcode('wpshop_myaccount', array(&$this, /*'wpshop_account_display_form'*/'wpshop_account_display_dashboard') );
				add_filter( 'wpshop_custom_template', array( &$this, 'custom_template_load' ) );
				
				wp_enqueue_script( 'wpshop_account_js', plugins_url('templates/wpshop/js/wpshop_account.js', __FILE__) );
				wp_enqueue_script( 'jquery_address', plugins_url('templates/wpshop/js/jquery.address-1.5.min.js', __FILE__) );
				
				add_action('wp_ajax_display_account_dashboard_section',array(&$this, 'wpshop_ajax_display_account_dashboard_section'));
				add_action('wp_ajax_save_address_form',array(&$this, 'wpshop_ajax_save_address_form'));
			...
			function managePost() {
				...
				if($wpshop->error_count()>0) {
					return array( false, $wpshop->show_messages() );
					//return false;
				}
				else {
					return array(true, '');
			 ...
			 */
			function display_addresses_dashboard( $from_front = false ) {
				global $wpdb;
				/** Header links : Log out and Edit personnal informations **/
				$tpl_component = array();
				...
				$billing_option = get_option('wpshop_billing_address');
				$address_dashboard .= self::get_addresses_by_type( $billing_option['choice'], __('Billing address', 'wpshop'), array(), $from_front );
				...
				if ( !empty($shipping_address_option['activate']) && ($shipping_address_option['activate'] == 'on')) {
					...
					if ( !empty($shipping_partner_option) && !empty($shipping_partner_option['activate']) && $shipping_partner_option['activate'] == 'on') {
						...
						$address_dashboard .= self::get_addresses_by_type( $shipping_address_option['choice'], __('Shipping address', 'wpshop'), array(), $from_front );
					}
				}
			 ...
			 */
			function get_addresses_by_type( $address_type_id, $address_type_title, $args = array(), $from_front = false ) {
				global $wpdb;
...
<p class="wpshop_customer_adress_edition_input_container wpshop_customer_adress_edition_input_container_' . $input_identifier . '">
	...
	<select name="user[' . strtolower($address_type) . '_info][' . $input_identifier . ']" id="wpshop_customer_adress_edition_input_' . strtolower($address_type) . '_' . $input_identifier . '" class="wpshop_customer_adress_edition_input wpshop_customer_adress_edition_input_' . $input_identifier . '" >
		<option value="">' . __('Choose...', 'wpshop') . '</option>';
			 ...
			 */
			function wpshop_account_display_dashboard () {
				if ( get_current_user_id() != 0 ) {
					$output = wpshop_display::display_template_element('wpshop_customer_account_dashboard', array(), array());
					echo $output;
				}
				else {
					echo $this->display_login_form();
				}
			}
			
			
			
			function wpshop_ajax_display_account_dashboard_section () {
				global $wpdb;
				$section = !empty( $_POST['dashboard_section'] ) ? wpshop_tools::varSanitizer( $_POST['dashboard_section'] ) : 'account_informations';
				$from_front = !empty( $_POST['from_front'] ) ? wpshop_tools::varSanitizer( $_POST['from_front'] ) : false;
				
				switch ( wpshop_tools::varSanitizer($section) ) {
					case '/account_informations' :
						$output = wpshop_display::display_template_element('wpshop_customer_account_infos_form', array('CUSTOMER_ACCOUNT_INFOS_FORM' => $this->display_account_form('', 'complete'), 'CUSTOMER_ACCOUNT_INFOS_FORM_BUTTONS' => '<input type="submit" name="submitOrderInfos" value="' . __('Save my account informations','wpshop') . '" />', 'CUSTOMER_ACCOUNT_INFOS_FORM_NONCE' => wp_create_nonce('wpshop_customer_register')));
					break;
					case '/account_orders' : 
						$output = $this->get_orders_list_for_customer( get_current_user_id() ) ;
					break;
					case '/account_address_book' : 
						$output = $this->display_addresses_dashboard( $from_front );
					break;
					case '/add_new_address_billing_address' :
						/** Check if there is the first address **/
						$query = $wpdb->prepare('SELECT * FROM ' . $wpdb->posts .' WHERE post_author = %d AND post_type = %s', get_current_user_id(), WPSHOP_NEWTYPE_IDENTIFIER_ADDRESS);
						$address_results = $wpdb->get_results( $query );
						$first = ( empty( $address_results ) ) ? true : false;
						
						$address_type = get_option('wpshop_billing_address');
						$output = $this->display_add_new_address_form( $address_type['choice'], $first, $form_front );
					break;
					case '/add_new_address_shipping_address' :
						$address_type = get_option('wpshop_shipping_address_choice');
						if ( $address_type['activate'] ) {
							$output = $this->display_add_new_address_form( $address_type['choice'] );
						}
					break;
					default : 
						$output = $this->display_account_form();
					break;
				}		
			...
			/**
			 * Return the customer's orders list
			 * @param integer $customer_id
			 * @return string
			 */
			function get_orders_list_for_customer ( $customer_id ) {
				global $wpdb;
				$user_id  = get_current_user_id();
				$query = $wpdb->prepare('SELECT ID FROM '.$wpdb->posts.' WHERE post_type = "'.WPSHOP_NEWTYPE_IDENTIFIER_ORDER.'" AND post_status = %s AND post_author = %d ORDER BY post_date DESC', 'publish', $user_id );
				...
				if ( !empty($orders_id) ) {
					...
					foreach ($orders_id as $o) {
						...
						if ( !empty($o['order_items']) && !empty($o['customer_id']) && ( $user_id == $o['customer_id'] ) ) {
							$sub_tpl_component = array();
							$sub_tpl_component['CUSTOMER_ACCOUNT_ORDER_KEY'] = ( !empty($o['order_key']) ) ? $o['order_key'] : null;
							$sub_tpl_component['CUSTOMER_ACCOUNT_ORDER_DATE'] = ( !empty($o['order_date']) ) ? $o['order_date'] : null;
							$sub_tpl_component['CUSTOMER_ACCOUNT_TOTAL_ATI'] = ( !empty($o['order_grand_total']) ) ? number_format($o['order_grand_total'], 2, '.', '') : null;
							$sub_tpl_component['CUSTOMER_ACCOUNT_ORDER_STATUS'] = ( !empty($order_status[$o['order_status']]) ) ? number_format($order_status[$o['order_status']], 2, '.', '') : null;
							$sub_tpl_component['CUSTOMER_ACCOUNT_ORDER_LINK'] = get_permalink(get_option('wpshop_myaccount_page_id')) . (strpos(get_permalink(get_option('wpshop_myaccount_page_id')), '?')===false ? '?' : '&amp;') . 'action=order&amp;oid='.$order_id;
						
							$orders .= wpshop_display::display_template_element('wpshop_customer_account_order', $sub_tpl_component, array());
							unset( $sub_tpl_component );
						}
					}
				}
				else {
					$orders =  __('No order', 'wpshop');
				}
				$output = wpshop_display::display_template_element('wpshop_customer_account_orders_section', array('CUSTOMER_ACCOUNT_ORDERS_LIST' => $orders), array());
				return $output;
			}
			 ...
			 * @param Boolean_type $first
			 * @return string
			 */
			function display_add_new_address_form ( $address_type, $first = false ) {
				$output = '';
				if ( $first) {
					$billing_address_option = get_option('wpshop_billing_address');
					$shipping_address_option = get_option('wpshop_shipping_address_choice');
					...
					$tpl_component = array();
					$tpl_component['CUSTOMER_ADDRESSES_FORM_CONTENT'] = $this->display_form_fields($billing_address_option['choice'], '', 'first');
						
					if ( $shipping_address_option['activate'] ) {
						$tpl_component['CUSTOMER_ADDRESSES_FORM_CONTENT'] .= '<p class="formField"><label><input type="checkbox" name="shiptobilling" checked="checked" /> '.__('Use as shipping information','wpshop').'</label></p>';
						$display = 'display:none;';
						$tpl_component['CUSTOMER_ADDRESSES_FORM_CONTENT'] .= '<div id="shipping_infos_bloc" style="'.$display.'">';
						$tpl_component['CUSTOMER_ADDRESSES_FORM_CONTENT'] .= $this->display_form_fields($shipping_address_option['choice'], '', 'first');
						$tpl_component['CUSTOMER_ADDRESSES_FORM_CONTENT'] .= '</div><br/>';
					}
					...
					$http_referer = ( !empty($_SERVER['HTTP_REFERER']) ) ? $_SERVER['HTTP_REFERER'] : '';
					$referer = ( !empty($_POST['referer']) ) ? $_POST['referer'] :  $http_referer;
					$output = $this->display_form_fields( $address_type, '', '', $referer );
					$output = wpshop_display::display_template_element('wpshop_customer_addresses_form', array('CUSTOMER_ADDRESSES_FORM_CONTENT' => $output, 'CUSTOMER_ADDRESSES_FORM_BUTTONS' => '<input type="submit" name="submitbillingAndShippingInfo" value="' . __('Save','wpshop') . '" />'));
				}
				return $output;
			}
			
			
			function wpshop_ajax_save_address_form (){
				$response = array();
				$status = false;
				$result = '';
				
				$managePost = $this->managePost();
				$status = $managePost[0];
				if ( !$status) {
					$response = $managePost[1];
				}
				else {
					$response = __('Your address is created', 'wpshop');
				}
				
				$response = array( 'status' => $status, 'reponse' => $response );
				echo json_encode($response);
				die();
			}
