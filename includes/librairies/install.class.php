...
class wpshop_install
{
	...
	*/
	function update_wpshop(){
		global $wpdb, $wpshop_db_table, $wpshop_db_table_list, $wpshop_update_way, $wpshop_db_content_add, $wpshop_db_content_update, $wpshop_db_options_add, $wpshop_eav_content, $wpshop_eav_content_update;
			...
			for($i = $lowest_version_to_execute; $i <= $current_def_max_version; $i++){
				...
				if(isset($wpshop_update_way[$i])){
					...
					if(is_array($wpshop_eav_content) && is_array($wpshop_eav_content[$i]) && (count($wpshop_eav_content[$i]) > 0)){
						...
						/*	Create attributes fo a given entity if attributes are set to be created for current version	*/
						...
						/*	Create attribute groups fo a given entity if attributes groups are set to be created for current version	*/
				...
				foreach($orders_id as $o){
					...
					if(!isset($myorder['order_tva'])){
						...
						$neworder = array(
							...
							'order_tva' => array_map('number_format_hack', $order_tva),//$order_tva,
							'order_items' => $items
						);
