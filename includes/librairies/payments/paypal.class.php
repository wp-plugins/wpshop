					...
					if (strcmp ($res, "VERIFIED") == 0) { 
						...
						if ($receiver_email == $paypalBusinessEmail) { 
							...
							if (empty($paypal_txn_id)) {
								...
								// On v�rifie que le montant pay� correspond au montant A payer..
								if ($amount_paid == $amount2pay ) {
								
									// On v�rifie que le statut du paiement est OK
	...
	function display_form($oid) {
		...
		if(!empty($order)) {
			...
			if(!empty($paypalBusinessEmail)) {
				...
				$currency = wpshop_tools::wpshop_get_currency($code=true); // Informations de commande � stocker
				...
				echo '
					<form action="'.$paypal.'" id="paypalForm" method="post">
						...
						<input id="invoice" name="invoice" type="hidden" value="'.$oid.'" /> <!-- Num�ro de facture -->
