...
wpshop(document).ready(function(){
	...
	jQuery(".addPrivateComment").live('click',function(){
		...
		if(comment != '') {
			...
			jQuery.getJSON(WPSHOP_AJAX_FILE_URL, { post: "true", elementCode: "ajax_addPrivateComment", oid: oid, comment:comment, send_email:send_email, send_sms:send_sms },
				function(data){
					_this.removeClass('loading');
					if(data[0]) jQuery('textarea[name=order_private_comment]').val('');
					alert(data[1]);
				}
			);
		}
	...
	/*jQuery("a.wpshop_infobulle_marker").mouseover(function(){
        if(jQuery(this).attr("title")=="") return false;
        
        jQuery("body").append('<span class="infobulle"></span>');
        var bulle = jQuery(".infobulle:last");
        bulle.append(jQuery(this).attr("title"));
        jQuery(this).attr("title","");
        var posTop = jQuery(this).offset().top;
        var posLeft = jQuery(this).offset().left;
        bulle.css({
            left:posLeft+20,
            top:posTop,
            opacity:0
    ...
    jQuery("a.wpshop_infobulle_marker").mouseout(function(){
        var bulle = jQuery(".infobulle:last");
        jQuery(this).attr("title",bulle.text());
        bulle.animate(
            {
                left:bulle.offset().left+20,
                opacity:0
            },
            500,
            "linear",
	...
	jQuery("a.wpshop_infobulle_marker").mouseover(function(){
        ...
        jQuery(this).attr("title","");
        var posTop = jQuery(this).offset().top;
        var posLeft = jQuery(this).offset().left+30;
